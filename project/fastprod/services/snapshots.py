...
def get_snapshot_by_name(name):
    ...
    for file in files:
        ...
        if name == file:
            snapshots.append(dict(file=file, path=os.path.join('fastprod/snapshots', file), at=file.split('_')[1].split('.')[0]))
    return snapshots
def create_snapshot_by_device(device=None):
    nr = app.config.get('nr')
    config = get_config_by_device(device)
    file_path = os.path.join('fastprod/snapshots/', '{0}_{1}.conf'.format(device.get('name'), datetime.now().strftime('%Y-%m-%d_%H-%M-%S')))
